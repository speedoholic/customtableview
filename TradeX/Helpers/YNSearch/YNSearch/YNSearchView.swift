...
open class YNSearchView: UIView, YNSearchMainViewDelegate, YNSearchListViewDelegate {
    ...
    open func ynSearchMainViewSearchHistoryChanged() {
        guard let clearButton = self.ynSearchMainView.clearHistoryButton else {return}
        let size = CGSize(width: self.frame.width, height: clearButton.frame.origin.y + clearButton.frame.height + 20)
