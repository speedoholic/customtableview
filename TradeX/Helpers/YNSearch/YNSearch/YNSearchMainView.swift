...
open class YNSearchMainView: UIView {
    ...
    open func initView(categories: [String]) {
        ...
        if let label = self.searchHistoryLabel {
    ...
    open func redrawSearchHistoryButtons() {
        ...
        let histories = Array(Set(ynSearch.getSearchHistories()))
        guard let searchLabel = searchHistoryLabel else {return}
        let searchHistoryLabelOriginY: CGFloat = searchLabel.frame.origin.y + searchLabel.frame.height
        ...
        if histories.count > 0 {
            ...
            if let clearButton = self.clearHistoryButton {
